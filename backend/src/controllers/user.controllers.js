...
const getConnections = async(req, res) => {
  ...
  try {
    ...
    if(user.connections.length == 0){
      return res.status(302).json({message:"user is not connected to any one at this moment!!"})
    }
    // const connectionList = user.connections.map(async (id) => {
    //   return await User.findById(id);
    //   // connectionList.push(connection.userName)
    // })
let connectionList = [];
let i = 0;
    do{
      const connection = await User.findById(user.connections[i]);
      connectionList.push(connection);
      i++;
    }while(i<user.connections.length);
...
const search = async (req, res) => {
  ...
  if(!userName || !query){
    return res.status(402).json({message:"search input filds is required"})
  }
  ...
  const user = await User.findOne({userName}).populate('connections');
  ...
  // Filter connections based on username query
  let connectedUsers = user.connections.filter(connection =>
    connection.userName.toLowerCase().includes(query.toLowerCase())
  );
  if(!connectedUsers){
    connectedUsers = user.connections.filter(connection =>
      connection.Name.toLowerCase().includes(query.toLowerCase())
    );
  }

  // const result = connectedUsers.concat(allUsers)

  return res.json({connectedUsers,allUsers});
